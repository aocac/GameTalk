...
<table>
  <tr>
    <td width="50%" align="center">
      ...
      <sub><b>好友管理</b>：按用户名 / #ID 添加 · 资料页 · 双击私聊</sub>
    </td>
    <td width="50%" align="center">
...
**现代界面**
- QQ NT 式三栏布局：图标导航栏 → 会话列表 → 聊天区，信息一目了然
- 会话选中高亮、未读/@我 角标、私聊在线状态点、深色导航栏

**游戏模式**
- 全局快捷键呼出输入框（可自定义录制，改键后旧键自动失效）
- 输入框顶部显示当前发送目标（#房间 / @好友），点击弹出列表随时切换
- Enter 发送或点击「发送」按钮 / Esc 取消（兼容中文输入法组词）
- 消息悬浮层：绝对透明背景、点击穿透、六种位置预设、可拖拽自定义、滚轮缩放
- 消息编辑 / 撤回实时同步到悬浮层，游戏内看到的一直是最新内容
- 发送后自动恢复游戏焦点（建议游戏使用无边框窗口化）

**群组通信**
- 房间制：创建房间 → 8 位邀请码邀请好友 → 实时群聊；一次加入永久保留（QQ 群式）
- 成员花名册：在线/离线状态（离线灰头像）、房主标注、点击查看资料卡片、右键菜单（@提及 / 加好友 / 房主禁言与移出）
- 好友管理（QQ 式）：添加好友折叠入口、申请区、好友列表与资料页；单击看资料、双击发消息
...
- 消息编辑：右键自己的消息即可修改，双端实时更新并显示「已编辑」标记（房间与私聊均支持）
- 撤回归属：谁撤的一目了然——自己撤「你撤回了一条消息」，房主代撤「房主撤回了 XX 的消息」
- 多房间并行：同时接收所有房间消息，未读角标 + @我 角标 + 最新消息预览实时更新

**通知与账号**
- Windows 系统通知：三档可调（全部消息 / 仅 @我 和私聊 / 不弹通知），在设置 → 通知中调整；提示音同页管理
...
升级不用先卸载：直接运行新安装包即可原地覆盖升级，运行中的旧版本会被自动关闭。
...
部署脚本会同时配置**每日数据库自动备份**（systemd 定时，保留 14 天），恢复方法见
...
├─ main    三栏聊天界面      ──REST(JWT)──►  REST 路由（认证 / 房间 / 好友 / 私聊）
...
- **测试**：server 52 + client 17 vitest 用例（含真实 WebSocket 集成测试）、ESLint、CI 全绿门禁
...
**怎么升级到新版本？**
直接下载新安装包运行即可原地覆盖升级，无需先卸载；聊天数据存在服务器，本机不丢任何东西。
