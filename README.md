...
- 房间用 8 位邀请码拉人，进了一次就是永久成员；也可以生成邀请链接——能设有效期、限使用次数、随时吊销——发给朋友点开网页就能加入
- 房主可以移出成员、限时禁言（10 分钟到 30 天）；每个房间有独立的群共享表情库，成员共同上传、全群可用，个人另有一份跨设备同步的云表情包
- 图片自动压缩上传（GIF 保动画），灯箱里缩放、保存、在会话的多张图之间翻看
- @提及打字时弹补全，被点到的有独立角标；引用、编辑（双端同步「已编辑」标记）、撤回（自己撤和房主代撤的文案不同）、转发（右键转到任意房间或好友，自动标注「来自哪里」）全支持
- 好友按用户名或 `#短 ID` 添加，私聊是独立的消息通道，非好友互相看不到历史；在线状态实时推送
- 消息按发送者连发自动分组，历史可以向上翻页，断线重连后自动补齐离线消息，每个会话有独立的输入草稿
