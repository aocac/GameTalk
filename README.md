...
| 邀请链接 | 房间右键菜单生成可过期、可限次数的邀请链接（`gametalk://` 深链，点击直接唤起客户端加入）；成员均可创建，创建者 / 房主可吊销 |
...
| 消息转发 | 右键任意可见消息转发到其他房间或好友；服务端代为复制并标注「来自 …」来源，引用与提及不带走 |
| 屏幕共享 | 房间内一键共享屏幕（WebRTC P2P，画面不经服务器），其他成员在窗口内直接观看；对称 NAT 场景可能受限（无 TURN 中继兜底） |
...
| 功能 | 说明 |
|---|---|
| Windows 系统通知 | 三档可调：全部消息 / 仅 @我 和私聊 / 不弹通知；点击通知自动跳到对应会话；正在输入时不打断 |
