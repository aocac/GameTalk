...
async function joinWs(ws: WebSocket, roomId: string): Promise<void> {
  ...
  await nextMessage(
    ws,
    (m) => m.type === "room:joined" && m.payload.roomId === roomId,
  );
}
...
describe("历史分页（修复：每页丢弃最新一条消息）", () => {
  it("房间历史：limit+2 条消息时首屏含最新一条，翻页不漏不重", async () => {
    ...
    // 全量拼接 = 5 条，无丢失无重复
    const all = [...page3.messages, ...page2.messages, ...page1.messages].map(
      (m: any) => m.text,
    );
...
describe("目标不在房间的错误码（修复：误让调用方丢房）", () => {
  ...
  it("信令目标不在房间 → target_not_in_room", async () => {
    ...
    await joinWs(ws, room.id);
    const err = nextMessage(ws, (m) => m.type === "error");
    sendWs(ws, "screen:signal", {
      roomId: room.id,
      to: "00000000-0000-4000-8000-000000000000",
      data: { type: "request", cid: "x" },
    });
...
describe("编辑重算提及（修复：编辑后 @ 不生效）", () => {
  it("编辑时新增 @成员 会写入 mentions 并随广播下发", async () => {
    ...
    sendWs(ws, "message:edit", {
      roomId: room.id,
      messageId: msg.id,
      text: "hello @reg_edit_target",
    });
    ...
    expect(hist.json().messages[0].mentions.map((m: any) => m.id)).toEqual([
      target.userId,
    ]);
    ws.close();
  });
...
describe("入参校验（修复：非法 id/类型 → 500）", () => {
  ...
  it("WS 非法 id / 非字符串文本 → invalid_input（不再 internal_error）", async () => {
    ...
    expect(["invalid_input", "target_not_in_room"]).toContain(
      (await e3).payload.code,
    );
    ws.close();
  });
});
  ...
  it("非法类型 / 超长 SDP / 非对象 data 一律拒绝", async () => {
    ...
    const cases: Array<Record<string, unknown>> = [
      ...
      { roomId: room.id, to: peer.userId, data: "not-an-object" },
      {
        roomId: room.id,
        to: peer.userId,
        data: { type: "offer", sdp: "x".repeat(20_000) },
      },
      {
        roomId: room.id,
        to: peer.userId,
        data: { type: "request", cid: "x".repeat(80) },
      },
    ];
    ...
    sendWs(ws, "screen:signal", {
      roomId: room.id,
      to: peer.userId,
      data: { type: "request", cid: "c1" },
    });
    expect((await atPeer).payload.data).toMatchObject({
      type: "request",
      cid: "c1",
    });
    ws.close();
...
describe("房间解散后的发送（修复：撞 FK 只回一句 internal error）", () => {
  it("房间被删除后发消息 → room_gone（含 roomId），且不误报 internal_error", async () => {
    const owner = await registerUser("reg_gone_owner");
    const room = await createRoom(owner.token, "解散房");
    const ws = await connectWs(owner.token);
    await joinWs(ws, room.id);

    const first = nextMessage(ws, (m) => m.type === "message:new");
    sendWs(ws, "message:send", { roomId: room.id, text: "解散前" });
    await first;

    // 模拟断线期间房间被解散（直连 SQL 绕过广播，客户端仍持有陈旧订阅）
    await db.query("DELETE FROM rooms WHERE id = $1", [room.id]);

    const err = nextMessage(ws, (m) => m.type === "error");
    sendWs(ws, "message:send", { roomId: room.id, text: "解散后" });
    const payload = (await err).payload;
    expect(payload.code).toBe("room_gone");
    expect(payload.roomId).toBe(room.id);
    expect(payload.message).toContain("解散");

    // 陈旧订阅已清理：再次发送在成员校验处即被拒（不再打到数据库），同样是可读错误码
    const again = nextMessage(ws, (m) => m.type === "error");
    sendWs(ws, "message:send", { roomId: room.id, text: "再试一次" });
    const againCode = (await again).payload.code;
    expect(["room_gone", "not_in_room"]).toContain(againCode);
    expect(againCode).not.toBe("internal_error");
    ws.close();
  });
});
