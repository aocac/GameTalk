...
function OverlayApp() {
  ...
  useEffect(() => {
    ...
    void listen<ChatMessage>('overlay:append', (e) => {
      ...
      clearTimers();
      const win = winRef.current;
      if (win && !adjusting) void win.show();
      scheduleHide();
  ...
  // 调整模式：纯 JS 拖拽（mousedown 记偏移 → mousemove setPosition → mouseup 保存）
  const onAdjustBarMouseDown = async (e: React.MouseEvent) => {
    ...
    const onMove = async (ev: MouseEvent) => {
      const nx = startPos.x + (ev.clientX - startMouse.x);
      const ny = startPos.y + (ev.clientY - startMouse.y);
