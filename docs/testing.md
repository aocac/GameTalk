...
| 范围 | 工具 | 内容 |
|---|---|---|
| 服务端单测/集成 | vitest | REST 路由、WS 网关（双客户端实时收发、房主删房、幂等 join、花名册/在线状态）、好友（申请/互加/删除 + 实时事件 + 在线广播）、好友私聊（非好友拒绝/双方广播/历史分页/会话聚合/撤回权限/图片与引用校验）、@提及（解析/成员校验/入库）、图片消息（上传校验/归属/绝对 URL）、禁言（仅房主/到期/解除）、WS 加固（限流 / 超大帧断连）、REST 限流（登录 429）、头像端点（data URL → HTTP 端点）、邀请链接（创建/资格/过期 410/耗尽 410/幂等入房不计数/吊销权限）、消息转发（源可见性/目标唯一/代复制 media/来源标注；转发进房复用禁言校验）、屏幕共享信令（非成员拒绝 / 启停广播 / 定向转发与越权拒绝）、migration 幂等、输入校验 |
| 客户端逻辑 | vitest | gameMode 管理器（mock Tauri，含换键后旧键注销）、真实 server 集成测试（双端聊天 / not_in_room / 断线重连 / 花名册离线保留 / 好友私聊收发与历史 / 编辑往返） |
...
| 19 | 好友私聊（非好友拒绝/双端实时/历史分页/会话聚合/仅发送者撤回/图片与引用校验） | ✅（6 例 + 客户端集成 1 例） | ✅ 双账号浏览器全流程（发起/互发/撤回/未读预览，docs/ui-dm-*.png） |
...
| 27 | 屏幕共享 P2P 信令（非成员拒绝发起 / 启停广播 / 定向转发与越权拒绝） | ✅（screen.test.ts，服务端信令） | ⬜ **需人类真机验收**：点顶栏「屏幕共享」是否弹系统选择器、跨网络 P2P 是否连通（Puppeteer 无法验证 WebView2 `getDisplayMedia` 行为） |

当前实测：server 65 测试（9 文件）+ client 17 测试（6 集成 + 11 单测）全绿（含邀请链接 / 消息转发 / 屏幕共享信令）；lint 双工作区通过；生产模式冒烟（health/register/login）通过。
...
> 以下为 2026-08-30 新增（v0.7.0），**尚待人类真机验收**：

- [ ] 屏幕共享：点顶栏「屏幕共享」按钮弹出系统屏幕选择器；双端跨网络 P2P 视频连通（服务端已验证信令透传，WebView2 采集与网络穿透需真机）
